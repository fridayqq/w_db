...
def search_wallets(conn, search_field, search_value):
    try:
...
def update_wallet(conn, wallet_id, field, new_value):
    try:
        cursor = conn.cursor()
...
def delete_wallet(conn, delete_field, delete_value):
    try:
        cursor = conn.cursor()
...
def add_wallet(conn, address, name):
    try:
        cursor = conn.cursor()
...
if st.button("Подключиться к базе данных"):
    ...
    if conn:
        ...
        if st.button("Получить данные из БД"):
            st.session_state.wallets_df = get_wallets(conn)
        ...
        if st.button("Найти записи"):
            search_results = search_wallets(conn, search_field, search_value)
        ...
        if st.button("Обновить запись"):
            update_wallet(conn, wallet_id, field_to_update, new_value)
        ...
        if st.button("Удалить запись"):
            delete_wallet(conn, delete_field, delete_value)
        ...
        if st.button("Добавить запись"):
            add_wallet(conn, new_address, new_name)

        conn.close()
        st.success("Соединение с базой данных закрыто.")
    else:
        st.error("Не удалось подключиться к базе данных. Проверьте введенные данные.")
